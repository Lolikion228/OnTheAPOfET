...
def create_transposed_dataframe_with_n(results, n_values):
    ...
    # Создаем DataFrame
    df = pd.DataFrame(data, index=index_names, columns=[f'h1 = {h2}' for h2 in h2_values])
...
def main():
    ...
    filename = './notes/v2_cauchy_res.txt'  # Замените на имя вашего файла
    results, n_values = parse_file(filename)
    df = create_transposed_dataframe_with_n(results, n_values)
    df.to_csv('./tex_shit/csvs/v2_cauchy.csv', columns=df.columns)

    filename = './notes/v2_normal_res.txt'  # Замените на имя вашего файла
    results, n_values = parse_file(filename)
    df = create_transposed_dataframe_with_n(results, n_values)
    df.to_csv('./tex_shit/csvs/v2_normal.csv', columns=df.columns)
