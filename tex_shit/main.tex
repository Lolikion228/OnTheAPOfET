...
\title{О проверке гипотезы равенства двух распределений}
...
\begin{equation}
	KS = \max_{1\leq i \leq N} | \hat{F}_m(Z_{(i)}) - \hat{G}_n(Z_{(i)})  |
\end{equation}
...
\begin{equation}
	CM = \frac{mn}{N^2} \sum_{i=1}^{N} (\hat{F}_m(Z_{(i)}) - \hat{G}_n(Z_{(i)}))^2
\end{equation}
...
\begin{table}[!h]
	...
	\begin{tabular}{l c c c c c c} 
		...
		& n=900 & \textbf{0.07} & \textbf{0.14} & 0.3 & 0.58 & 0.81 \\ 
		\addlinespace
		\addlinespace
		\multirow{3}{*}{CM} 
		...
		& n=900 & \textbf{0.07} & \textbf{0.14} & 0.33 & 0.58 & 0.81 \\ 
		\addlinespace
		\addlinespace
		\multirow{3}{*}{KS} 
